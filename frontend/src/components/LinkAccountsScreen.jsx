...
export function LinkAccountsScreen() {
  ...
  const generateQRCode = async (forceRefresh = false) => {
    ...
    try {
      ...
      const url = forceRefresh 
        ? `${getApiUrl('api/whatsapp/qrcode')}?refresh=true`
        : getApiUrl('api/whatsapp/qrcode');
  ...
  const handleWhatsAppConnect = () => {
    // Force refresh QR code when button is clicked
    generateQRCode(true);
  };

  // Check WhatsApp connection status on mount (but don't auto-generate QR)
  useEffect(() => {
    let isMounted = true;
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (statusData.connected) {
            ...
            // If not connected, just clear connection details (don't auto-generate QR)
    ...
    // Initial status check only (no auto QR generation)
    checkStatus();
            ...
            >
              {qrCodeLoading ? (
                ...
                <>
                  ...
                  <span>{qrCodeData && !qrCodeExpired ? 'Refresh QR Code' : 'Generate QR Code'}</span>
                </>
              )}
            </button>
            </div>
